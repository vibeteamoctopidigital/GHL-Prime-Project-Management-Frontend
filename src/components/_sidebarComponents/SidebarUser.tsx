...
import { LogOut, Shield, Briefcase, Star, User } from 'lucide-react';
...
export default function SidebarUser({ currentUser, logout, onClose }: SidebarUserProps) {
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      case 'CEO': return 'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-500/20 dark:text-purple-300 dark:border-purple-500/30';
      case 'HR': return 'bg-teal-100 text-teal-700 border-teal-200 dark:bg-teal-500/20 dark:text-teal-300 dark:border-teal-500/30';
      case 'DEPT HEAD': return 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-300 dark:border-emerald-500/30';
      case 'Team Lead': return 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-500/20 dark:text-amber-300 dark:border-amber-500/30';
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      case 'CEO': return <Shield className="w-3 h-3" />;
      case 'HR': return <Briefcase className="w-3 h-3" />;
      case 'DEPT HEAD': return <Shield className="w-3 h-3" />;
      case 'Team Lead': return <Star className="w-3 h-3" />;
